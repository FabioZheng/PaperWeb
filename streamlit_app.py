...
def render_model_lineup(active_roles: set[str] | None = None, container=None) -> None:
    active_roles = active_roles or set()
    target = container if container is not None else st
    target.subheader("Model lineup")
    cols = target.columns(5)
    ...
    for i, role in enumerate(roles):
        ...
        if role in active_roles:
            cols[i].markdown(
                f"<div style='padding:8px;border:2px solid #22c55e;border-radius:8px;background:#fef9c3'><b>{role}</b><br/>{label}<br/><span style='color:#a16207'>● running</span></div>",
...
render_model_lineup(set(st.session_state.get("active_roles", [])), container=lineup_placeholder)
...
if submitted:
    ...
    def _refresh_lineup() -> None:
        render_model_lineup(live_roles, container=lineup_placeholder)

    def on_event(msg: str) -> None:
        logs.append(msg)
        status.info(msg)
        if "[extractor:start]" in msg:
            live_roles.add("extractor")
            _refresh_lineup()
        elif "[extractor:end]" in msg:
            live_roles.discard("extractor")
            _refresh_lineup()
