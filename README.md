...
## Multi-source ingestion and freshness tracking
...
Example command:
```bash
python -m app.ingest --query "agentic information retrieval evaluation" --sources arxiv,semantic_scholar,openalex --limit 50
```

What happens in this ingestion pass:
1. Selected source connectors fetch and normalize metadata into one `PaperMetadata` schema.
2. Deduplication resolves identity in order: DOI, arXiv ID, OpenReview ID, Semantic Scholar ID, OpenAlex ID, then normalized-title fallback.
3. Metadata enrichment merges non-empty/richer fields and preserves `source_seen_history`.
4. Freshness/version tracking updates `first_seen_at`, `last_seen_at`, `version`, `metadata_hash`, `pdf_hash`, `is_new`, and `is_updated`.

The command prints a summary with fetched/new/updated/merged/failed counts.

Environment variables:
- `PAPERWEB_CONFIG` (optional config file override)
- `OPENAI_API_KEY` (optional, only for OpenAI provider usage)
